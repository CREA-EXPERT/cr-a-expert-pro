...
export const supprimerMonCompte = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    try {
      ...
      if (error) throw new Error("suppression_auth");
      ...
      await journaliser({
        ...
        action: "suppression",
        resultat: "echec",
        codeErreur: e instanceof Error ? e.message.slice(0, 60) : "inconnu",
        nbElements: ids.length,
      });
      throw new Error("La suppression du compte n'a pas pu être menée à son terme.");
    }
  });
